 ...
 *			    do_setgroup			       *
 *=====================================================================*/
PUBLIC int do_setgroup(m_ptr)
register message *m_ptr;
{
  int proc_nr = m_ptr->m1_i1;
  int group = m_ptr->m1_i2;
  int estimate = m_ptr->m1_i3;
  
  register struct proc *rp = proc_addr(proc_nr);
 

  if (!isuserp(rp)) return EINVAL;
  
  rp->sched_group = group;
  rp->wait_ticks = 0;
  rp->rr_counter = 0;
  
  if (group == 3) {
  	rp->cpu_estimate = estimate;
  } else
  {
  	rp->cpu_estimate = 100;
  }
  
  printf("KERNEL: do_setgroup: proc=%d, group=%d, estimate=%d\n",
           m_ptr->m1_i1, rp->sched_group, rp->cpu_estimate);          
