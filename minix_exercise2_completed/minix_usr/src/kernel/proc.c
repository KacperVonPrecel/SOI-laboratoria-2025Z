...
PRIVATE struct proc *g1_queue = NIL_PROC;
PRIVATE struct proc *g2_queue = NIL_PROC;
PRIVATE struct proc *g3_queue = NIL_PROC;

PRIVATE struct proc *g1_tail = NIL_PROC;
PRIVATE struct proc *g2_tail = NIL_PROC;
PRIVATE struct proc *g3_tail = NIL_PROC;
 ...
 */
  ...
  if ( (rp = g1_queue) != NIL_PROC) {
  	proc_ptr = rp;
  	bill_ptr = rp;
  	return;
  }
  if ( (rp = g2_queue) != NIL_PROC) {
  	struct proc *current = g2_queue;
  	
  	while (current != NIL_PROC) {
  		if (current->wait_ticks > rp->wait_ticks) {
  			rp = current;
  		}
 ...
 *                         ready_with_group				     * 
 *===========================================================================*/
PRIVATE void ready_with_group(rp)
register struct proc *rp;
{
   struct proc *prev = NIL_PROC;
   struct proc *current = g3_queue;
   
   switch(rp->sched_group)
   {
   	case 1: /* Round Robin for group 1 */
   		if (g1_queue == NIL_PROC)
   		{
   			g1_queue = rp;
   			g1_tail = rp;
   		} else 
   		{
   			g1_tail->p_nextready = rp;
   			g1_tail = rp;
   		}
   		rp->p_nextready = NIL_PROC;   		
   		break;
   	
   	case 2: /* Aging for group 2 */
   		if (g2_queue == NIL_PROC)
		{
			g2_queue = rp;
			g2_tail = rp;
		} else
		{
			g2_tail->p_nextready = rp;
			g2_tail = rp;
		}
		rp->p_nextready = NIL_PROC;		
		break;
	...
	case 3: /* SJF */
		/* In here we insert the process into queue sorted by lowest cpu_estimate */
		while (current != NIL_PROC && current->cpu_estimate < rp->cpu_estimate)
		{
			prev = current;
			current = current->p_nextready;
		}
		...
		if (prev == NIL_PROC)
		{
			rp->p_nextready = g3_queue;
			g3_queue = rp;
			if (g3_queue->p_nextready == NIL_PROC) 
				g3_tail = rp;
		} else
		{
			rp->p_nextready = prev->p_nextready;
			prev->p_nextready = rp;
 ...
 */
  ...
  if (isuserp(rp) && rp->sched_group > 0 && rp->sched_group < 4) {
  	ready_with_group(rp);
  	return;
  }
 ...
 *		 	  unready_from_group			  	     * 
 *===========================================================================*/
PRIVATE void unready_from_group(rp, head_ptr, tail_ptr)
register struct proc *rp;
struct proc **head_ptr;
struct proc **tail_ptr;
{
   register struct proc *xp;
   
   if (*head_ptr == NIL_PROC) return;
   
   if (*head_ptr == rp)
   {
   	*head_ptr = rp->p_nextready;
   	if (*head_ptr == NIL_PROC) *tail_ptr = NIL_PROC;
   	return;
   }
...
/* A process has blocked. */
  ...
  } else if (isuserp(rp) && rp->sched_group > 0 && rp->sched_group < 4) {
  	switch(rp->sched_group) {
  	    case 1:
                unready_from_group(rp, &g1_queue, &g1_tail);
                break;
            case 2:
                unready_from_group(rp, &g2_queue, &g2_tail);                
                break;
            case 3:
                unready_from_group(rp, &g3_queue, &g3_tail);                
                break;
        }
        if (rp == proc_ptr) {
        	pick_proc();
...
/*===========================================================================*
 *			  update_aging_for_group2			     * 
 *===========================================================================*/
PRIVATE void update_aging_for_group2()
{
   struct proc *rp;
   ...
   while (rp != NIL_PROC) {
   	if (rp != proc_ptr) {
   		...
   		if (rp->wait_ticks > MAX_WAIT_TICKS) {
   			rp->wait_ticks = MAX_WAIT_TICKS;
   		}
   	}
   	rp = rp->p_nextready;
   }
   ...
   for (rp = &proc[0]; rp < &proc[NR_TASKS + NR_PROCS]; rp++) {
   	if (isuserp(rp) && rp->sched_group == 2 && (rp->p_flags & (SENDING | RECEIVING))) {
   		rp->wait_ticks++;
   		if (rp->wait_ticks > MAX_WAIT_TICKS) {
   			rp->wait_ticks = MAX_WAIT_TICKS;
   		}
...
{   
   ...
   if (isuserp(rp) && rp->sched_group == 1) {
   	rp->rr_counter = rp->rr_counter + 1;
   	if (rp->rr_counter >= RR_QUANTUM) {   		
   		rp->rr_counter = 0;
   		lock_sched();
   	}
   }
 ...
 */
  struct proc *current = proc_ptr;

  if (isuserp(current) && current->sched_group == 1) {  	
  	if (g1_queue != NIL_PROC && g1_queue->p_nextready != NIL_PROC) {
  		struct proc *next_rr = g1_queue->p_nextready;  		
  		if (g1_tail != current) {
  			g1_tail->p_nextready = current;
  		}
  		g1_tail = current;
  		current->p_nextready = NIL_PROC;
  		current->rr_counter = 0;
  		g1_queue = next_rr;
  		pick_proc();
  		return;
	}
