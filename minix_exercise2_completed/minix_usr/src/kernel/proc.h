...
PUBLIC void handle_clock_tick(void);
...
struct proc {
  ...
  int sched_group;		/* 1, 2 or 3 */
  clock_t cpu_estimate;		/* estimated time of CPU for group 3 */
  clock_t wait_ticks;		/* waiting tick counter for group 2 */
  int rr_counter;		/* time counter in quantum for rr */
};
