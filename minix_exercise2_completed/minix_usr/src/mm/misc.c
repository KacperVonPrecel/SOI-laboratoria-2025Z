...
PUBLIC int do_setgroup()
{
   ...
   who = mm_in.m1_i1;
   group = mm_in.m1_i2;
   estimate = mm_in.m1_i3;
   
   if (who == 0) {
        /* Dla bieżącego procesu */
        proc_nr = mp - mproc;  /* mp to wskaźnik do bieżącego mproc */
    } else {
        /* Znajdź po PID */
        for (proc_nr = 0; proc_nr < NR_PROCS; proc_nr++) {
            if (mproc[proc_nr].mp_pid == who) {
                break;
            }
        }
        if (proc_nr >= NR_PROCS) return ESRCH;
    }
    
   if (group < 1 || group > 3) return EINVAL;
   if (group == 3 && estimate <= 0) return EINVAL;
    
   rmp = &mproc[proc_nr];
    
   rmp->mp_sched_group = group;    

   m.m_type = SYS_SETGROUP;
   m.m1_i1 = proc_nr;
   m.m1_i2 = group;
   m.m1_i3 = estimate;
    
   result = _taskcall(SYSTASK, SYS_SETGROUP, &m);
