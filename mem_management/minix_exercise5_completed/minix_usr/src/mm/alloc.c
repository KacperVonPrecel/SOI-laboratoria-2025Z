...
PUBLIC int do_hole_map()
{
  ...
  char *user_buf_ptr = (char *)mm_in.m1_p1;
  int user_buf_size = mm_in.m1_i1;
  
  /* Setting up a limit for buffor */
  int user_max_pairs = user_buf_size / ( 2 * sizeof(unsigned int));
  int limit = (user_max_pairs < 512) ? user_max_pairs : 512;
  ...
  /* List all available holes */
  while( h != NIL_HOLE && count < limit - 1) {  
  	buffer[ptr++] = h->h_len;
  	buffer[ptr++] = h->h_base;
  	count++;
  	h = h->h_next;
  ...
  bytes_to_copy = ptr * sizeof(unsigned int);
  
  res = sys_copy(MM_PROC_NR, D, (phys_bytes)buffer,
                 who, D, (phys_bytes)user_buf_ptr,
                 bytes_to_copy);
     
  if (res != OK) {
        return res;  /* błąd kopiowania */
    }
 ...
 */
  ...
  register struct hole *h, *prev_h;
  struct hole *best_h, *best_prev;
  ...
  if (wf_mode == 1) {
  	...
  	if (best_h != NIL_HOLE) {
  		old_base = best_h->h_base;
  		best_h->h_base += clicks;
  		best_h->h_len -= clicks;
  		
  		if (best_h->h_len == 0) {
  			del_slot(best_prev, best_h);
  		}
  		return(old_base);
  	}	
  	return(NO_MEM);
